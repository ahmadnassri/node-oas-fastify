...
module.exports = function (instance, opts, next) {
  ...
  const securitySchemas = (opts.spec && opts.spec.components.securitySchemes)
    ? opts.spec.components.securitySchemes
    : {}
  ...
  for (const path in opts.spec.paths) {
    ...
    for (const method in pathSpec) {
      ...
      const methodSecurity = oasRoute.security || globalSecurity
      ...
      const fastifyRoute = {
        ...
        schema: {
          ...
          query: security(parameters(oasRoute.parameters, 'query'), 'query', securitySchemas, methodSecurity),
          headers: security(parameters(oasRoute.parameters, 'header'), 'header', securitySchemas, methodSecurity),
          response: {}
        }
      }

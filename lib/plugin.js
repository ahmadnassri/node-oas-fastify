...
const stripProperties = require('./helpers/strip-properties')
...
module.exports = function (instance, opts, next) {
  ...
  for (const path in opts.spec.paths) {
    ...
    for (const method in pathSpec) {
      ...
      if (oasRoute?.requestBody?.content) {
        // get the first one
      ...
      for (const status in oasRoute.responses) {
        ...
        if (responseSpec?.content?.['application/json']?.schema) {
          ...
          fastifyRoute.schema.response[status] = responseSchema
        }
      }
      ...
      fastifyRoute.schema.params = stripProperties(fastifyRoute.schema.params)
      fastifyRoute.schema.query = stripProperties(fastifyRoute.schema.query)
      fastifyRoute.schema.headers = stripProperties(fastifyRoute.schema.headers)
      if (fastifyRoute.schema.body) {
        fastifyRoute.schema.body = stripProperties(fastifyRoute.schema.body)
      }
      Object.keys(fastifyRoute.schema.response).forEach((item) => {
        fastifyRoute.schema.response[item] = stripProperties(fastifyRoute.schema.response[item])
      })
